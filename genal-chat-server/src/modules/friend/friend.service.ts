...
export class FriendService {
  ...
  async getFriends(userId: string) {
    try {
      if(userId) {
        return { msg:'Nhận được sự thành công của bạn bè người dùng', data: await this.friendRepository.find({userId: userId}) };
      } else {
        return { msg:'Có được bạn bè người dùng không thành công', data: await this.friendRepository.find() };
      }
    } catch(e) {
      return { code:RCode.ERROR, msg:'Có được bạn bè người dùng không thành công', data:e };
    }
  }
