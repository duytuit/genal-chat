    ...
    <div v-if="type === 'group'">
      ...
      >
        <div class="active-content" v-if="activeGroupUser[activeRoom.groupId]">
          <div class="actiev-content-title">Quản lý nhóm</div>
          <div class="active-content-sum">người dùng trực tuyến: {{ activeNum }}</div>
          ...
          <a-button type="danger" class="active-content-out" @click="exitGroup">Xóa</a-button>
        </div>
      </a-drawer>
    </div>
    <div v-else>
      <a-popconfirm title="Bạn có chắc chắn muốn xóa không?" placement="bottomRight" ok-text="Yes" cancel-text="No" @confirm="exitFriend">
