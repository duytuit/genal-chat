...
export default class GenalInput extends Vue {
  ...
  throttle(fn: Function, file?: File) {
    ...
    if (nowTime - this.lastTime < 500) {
      return this.$message.error('Thông điệp quá thường xuyên!');
    }
    fn(file);
  ...
  preSendMessage() {
    ...
    if (this.text.length > 1000) {
      this.$message.error('Tin tức quá dài!');
      return;
    }
  ...
  async handleImgUpload(imageFile: File) {
    ...
    if (!isJpgOrPng) {
      return this.$message.error('xin vui lòng chọn jpeg/jpg/png/gif Định dạng hình ảnh!');
    }
