<template>
  <div class="tool">
    ...
    <a-tooltip placement="topLeft" arrow-point-at-center>
      <div slot="title">
        <div>Vui lòng trò chuyện văn minh</div>
        <div>Ảnh chụp màn hình có thể gửi hình ảnh</div>
      </div>
    ...
    <a-modal title="thông tin người dùng" :visible="showUserModal" footer="" @cancel="showUserModal = false">
      <div class="tool-user">
        ...
        <div class="tool-user-info">
          <div class="tool-user-title">Thay đổi tên người dùng</div>
          <a-input class="tool-user-input" v-model="username" placeholder="Vui lòng nhập tên người dùng"></a-input>
          <a-button type="primary" @click="changeUserName">xác nhận</a-button>
        </div>
        <div class="tool-user-info">
          <div class="tool-user-title">đổi mật khẩu</div>
          <a-input-password class="tool-user-input" v-model="password" placeholder="Xin vui lòng nhập mật khẩu"></a-input-password>
          <a-button type="primary" @click="changePassword">xác nhận</a-button>
        </div>
      </div>
    </a-modal>
    <a-modal title="Chủ đề" :visible="showBackgroundModal" footer="" @cancel="showBackgroundModal = false">
      <div class="tool-user-info">
        <div class="tool-user-title" style="width: 65px;">
          <span>Lý lịch</span>
          <a-tooltip placement="topLeft" arrow-point-at-center>
            <div slot="title">
              <span>Khi vào không gian, nền mặc định, hỗ trợ jpg, png, gif Định dạng bằng nhau</span>
            </div>
            <a-icon type="exclamation-circle" style="margin-left: 5px;" />
          </a-tooltip>
        </div>
        <a-input v-model="background" class="tool-user-input" placeholder="Vui lòng nhập trang web hình nền"></a-input>
...
export default class GenalTool extends Vue {
  ...
  beforeUpload(file: any) {
    ...
    if (!isJpgOrPng) {
      return this.$message.error('Vui lòng tải lên jpeg/jpg/png/gif Định dạng hình ảnh!');
    }
    const isLt1M = file.size / 1024 / 1024 < 0.5;
    if (!isLt1M) {
      return this.$message.error('Hình ảnh phải ít hơn 500K!');
    }
    this.avatar = file;
    this.handleUpload();
    return false;
