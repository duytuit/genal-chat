...
export function processReturn(res: AxiosResponse<ServerRes>) {
  // code 0:thành công 1:sai lầm 2:Lỗi phụ trợ
...
export function nameVerify(name: string): boolean {
  ...
  //   Vue.prototype.$message.error('Tên chỉ chứa các ký tự Trung Quốc, chữ cái, số, số và dòng hướng xuống.');
  //   return false;
  // }
  if (name.length > 50) {
    Vue.prototype.$message.error('Tên nhóm quá dài');
    return false;
  }
  return true;
}
...
export function passwordVerify(password: string): boolean {
  ...
  if (!passwordReg.test(password)) {
    Vue.prototype.$message.error('Mật khẩu chỉ chứa các chữ cái, số và cấp dưới');
    return false;
  }
